...
export default async function Page() {
  async function checkIsUserIsInCurrentVersionAndUpdateCookieToNewVersion(
    authToken: string
  ) {
  ...
  const req = await fetch(
    `${process.env.API_APONTAMENTO_URL}/get-permissions`,
    {
      headers: {
  ...
  if (req.ok) {
    const userAlreadyViewedReleaseNotes =
      await checkIsUserIsInCurrentVersionAndUpdateCookieToNewVersion(authToken);
