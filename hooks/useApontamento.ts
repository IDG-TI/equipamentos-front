...
export default function useApontamento() {
    ...
    const [isLastApontamentoLoading, setIsLastApontamentoLoading] = useState(false);
    const isSecaoJornadaEspecifica = useState(false);

    function getHoras() {
        const validHoraEntrada = horaEntrada[0] && parseFloat(horaEntrada[0]) >= 0;
        const validHoraSaida = horaSaida[0] && parseFloat(horaSaida[0]) >= 0;

        if(validHoraEntrada && validHoraSaida && isSecaoJornadaEspecifica[0]) {
            const horasTrabalhadas = parseFloat(horaEntrada[0]) == parseFloat(horaSaida[0]) ?  24 : (horaEntrada[0] > horaSaida[0] ? 24 + parseFloat(horaSaida[0]) - parseFloat(horaEntrada[0]) : parseFloat(horaSaida[0]) - parseFloat(horaEntrada[0]));
            console.log("horasTrabalhadas", horasTrabalhadas);
            horas[1](horasTrabalhadas);
    ...
    const getBodySubmit = () => {
        return {
            ...
            "horaEntrada": secao[0] && secao[0].dadosComplementares.jornadaEspecifica ? horaEntrada[0] : null,
            "horaSaida": secao[0] && secao[0].dadosComplementares.jornadaEspecifica ? horaSaida[0] : null,
            "ld": ld[0],
    ...
    function checkSecaoIsJornadaEspecifica(secao : Secao | null) {
        if (secao && secao.dadosComplementares && secao.dadosComplementares.jornadaEspecifica) {
    ...
    useEffect(function loadProjetoAtividades() {
        if (projeto[0] && projeto[0].id) {
            projetoAtividades.reload(`projeto/atividade-apontamento/${projeto[0].id}`);
        }
    }, [projeto[0]]);
    
    useEffect(function filterProjetoAtividadesInAtividadesList() {
        if (projetoAtividades.options) {
            atividades.filter((item: any) => projetoAtividades.options.includes(item.id));
        }
    }, [projetoAtividades.options]);

    useEffect(function getSecaoInfo() {
